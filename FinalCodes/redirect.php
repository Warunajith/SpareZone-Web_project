...
session_start();
include('db.php');
//$userid=$con->query("SELECT `user_id` FROM temporary");
//$user_id=$user_id->fetch_arrray();

//Updating the items table using temporary cart table data

$temporary=$con->query("SELECT * FROM temporary");

while($row=$temporary->fetch_array()){

    $stock=$con->query("SELECT stock FROM `items` WHERE code='".$row['code']."'");
    $stock=$stock->fetch_array();
    $stock=intval($stock[0])-$row['quantity'];
    
    $con->query("UPDATE `items` SET stock=$stock WHERE code='".$row['code']."'");
...
//Adding data into the Sales table using temporary table which has cart data


$next_id=$con->query("SELECT MAX(id) FROM sales");
$next_id=$next_id->fetch_array();
$next_id=intval($next_id[0]);
$next_id++;

$sale_price=$con->query("SELECT SUM(price) FROM temporary");
$sale_price=$sale_price->fetch_array();
$sale_price=intval($sale_price[0]);

$sale_quan=$con->query("SELECT SUM(quantity) FROM temporary");
$sale_quan=$sale_quan->fetch_array();
$sale_quan=intval($sale_quan[0]);


$userid=$con->query("SELECT `user_id` FROM temporary");
$c_id=$userid->fetch_array();

$date=date("Y-m-d");

$sql="INSERT INTO sales values ('$next_id','$sale_price','$c_id[0]','$sale_quan','$date')";

    $con->query($sql);

// updating Sales table ended
...
<div class="container">
                ...
                <!--Payment End gateway form here-->
 
    <div class="row">
        <div class="col-sm">

            <div class="card bg-light text-dark" style="width:500px; height:250px;margin-left:250px;">
                <div class="card-body">
                
                    <br>
                    <div style="text-align: center;">
                  
                        <button type="button" class="btn btn-success"  style="width:400px; height:50px;font-size:25px;">
                ...
                <button type="button" class="btn btn-warning"  style="width:400px; height:50px;font-size:25px;">
